...
if(store_likelihoods == TRUE) {
  ...
  duration_lik <- rowSums(duration_lik)
  empl_lik <- rowSums(empl_lik)
  p_tenure_q1 <- rowSums(p_tenure_q1)
  p_tenure_q2 <- rowSums(p_tenure_q2)
  p_tenure_q3 <- rowSums(p_tenure_q3)
  p_unempl_dur_q1 <- rowSums(p_unempl_dur_q1)
  p_unempl_dur_q2 <- rowSums(p_unempl_dur_q2)
  p_unempl_dur_q3 <- rowSums(p_unempl_dur_q3)

  print(paste0("Total: ", sum(llik)))
  print(paste0("Empl: ", sum(empl_lik)))
  print(paste0("Duration: ", sum(duration_lik)))
  print(paste0("Tenure1: ", sum(p_tenure_q1)))
  print(paste0("Tenure2: ", sum(p_tenure_q2)))
  print(paste0("Tenure3: ", sum(p_tenure_q3)))
  print(paste0("Unempl_dur1: ", sum(p_unempl_dur_q1)))
  print(paste0("Unempl_dur2: ", sum(p_unempl_dur_q2)))
  print(paste0("Unempl_dur3: ", sum(p_unempl_dur_q3)))
...
get_ar1_tenure_joint_probability_two_sigma <- function(
  ...
  lp_misclass_1 <- ifelse(is.na(lp_misclass_1)|is.infinite(lp_misclass_1), log(0.0000001), lp_misclass_1)

  lp_misclass_2 <- log(p_misclass_2)
  lp_misclass_2 <- ifelse(is.na(lp_misclass_2)|is.infinite(lp_misclass_2), log(0.0000001), lp_misclass_2)

  lp_misclass_3 <- log(p_misclass_3)
  lp_misclass_3 <- ifelse(is.na(lp_misclass_3)|is.infinite(lp_misclass_3), log(0.0000001), lp_misclass_3)

  lp_employed_q1 <- log(p_employed_q1)
  lp_employed_q1 <- ifelse(is.na(lp_employed_q1)|is.infinite(lp_employed_q1), log(0.0000001), lp_employed_q1)

  lp_trans_q2 <- log(p_trans_q2)
  lp_trans_q2 <- ifelse(is.na(lp_trans_q2)|is.infinite(lp_trans_q2), log(0.0000001), lp_trans_q2)

  lp_trans_q3 <- log(p_trans_q3)
  lp_trans_q3 <- ifelse(is.na(lp_trans_q3)|is.infinite(lp_trans_q3), log(0.0000001), lp_trans_q3)

  lp_tenure_q1 <- log(p_tenure_q1)
  lp_tenure_q1 <- ifelse(is.na(lp_tenure_q1)|is.infinite(lp_tenure_q1), log(0.0000001), lp_tenure_q1)

  lp_unempl_dur_q1 <- log(p_unempl_dur_q1)
  lp_unempl_dur_q1 <- ifelse(is.na(lp_unempl_dur_q1)|is.infinite(lp_unempl_dur_q1), log(0.0000001), lp_unempl_dur_q1)

  lp_tenure_q2 <- log(p_tenure_q2)
  lp_tenure_q2 <- ifelse(is.na(lp_tenure_q2)|is.infinite(lp_tenure_q2), log(0.0000001), lp_tenure_q2)

  lp_unempl_dur_q2 <- log(p_unempl_dur_q2)
  lp_unempl_dur_q2 <- ifelse(is.na(lp_unempl_dur_q2)|is.infinite(lp_unempl_dur_q2), log(0.0000001), lp_unempl_dur_q2)

  lp_tenure_q3 <- log(p_tenure_q3)
  lp_tenure_q3 <- ifelse(is.na(lp_tenure_q3)|is.infinite(lp_tenure_q3), log(0.0000001), lp_tenure_q3)

  lp_unempl_dur_q3 <- log(p_unempl_dur_q3)
  lp_unempl_dur_q3 <- ifelse(is.na(lp_unempl_dur_q3)|is.infinite(lp_unempl_dur_q3), log(0.0000001), lp_unempl_dur_q3)
